            ...
            while True:
                ...
                try:
                    ...
                    if is_rate_limit:
                        ...
                        is_free_tier = ":free" in model_name.lower() or "free" in err_str
                    ...
                    if should_retry:
                        ...
                        if retry_delay > max_retry_timeout:
                            ...
                            if is_rate_limit:
                                model_name = self.main_model.name if hasattr(self, "main_model") else ""
                                is_free_tier = ":free" in model_name.lower()
                                ...
                                if is_free_tier:
                                    ...
                                    self.io.tool_error(
                                        "   Free tier models have very strict rate limits. Options:"
                                    )
                                    self.io.tool_error("   1. Switch to a different model: /model <model-name>")
                                    self.io.tool_error(
                                        "   2. Add your OpenRouter API key for better rate limits"
                                    )
                                    self.io.tool_error(
                                        "   3. Wait 5-10 minutes before retrying this model"
                                    )
                                else:
                                    self.io.tool_error(
