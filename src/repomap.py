...
from src.special import filter_important_files
from src.waiting import Spinner
...
class RepoMap:
    ...
    def get_tags_raw(self, fname, rel_fname):
        ...
        # In tree-sitter 0.20+, use Query constructor and QueryCursor to execute
        from tree_sitter import Query, QueryCursor
        query = Query(language, query_scm)
        # Use QueryCursor to execute the query
        cursor = QueryCursor(query)
        # matches() returns list of (pattern_index, {capture_name: [nodes]}) tuples
        matches = cursor.matches(tree.root_node)
        # Extract captures from matches - matches are (pattern_index, captures_dict)
        captures = {}
