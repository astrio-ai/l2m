...
class GitRepo:
    repo = None
    l2m_ignore_file = None
    l2m_ignore_spec = None
    l2m_ignore_ts = 0
    l2m_ignore_last_check = 0
    subtree_only = False
    ignore_file_cache = {}
    git_repo_error = None
    ...
    def __init__(
        ...
        if l2m_ignore_file:
            self.l2m_ignore_file = Path(l2m_ignore_file)

    def commit(self, fnames=None, context=None, message=None, l2m_edits=False, coder=None):
        """
        ...
        Args:
            ...
            l2m_edits (bool, optional): Whether the changes were made by L2M. Defaults to False.
        ...
        L2M generated the changes (`l2m_edits`).
        ...
        - l2m_edits=True: Changes were generated by L2M (LLM).
        - l2m_edits=False: Commit is user-driven (e.g., /commit manually staged changes).
        ...
        Flags:
        - --attribute-author: Modify Author name to "User Name (l2m)".
        - --attribute-committer: Modify Committer name to "User Name (l2m)".
        - --attribute-co-authored-by: Add
          "Co-authored-by: l2m (<model>) <l2m@l2m.chat>" trailer to commit message.

        Behavior Summary:

        1. When l2m_edits = True (AI Changes):
        ...
        - AI edit with Co-authored-by + Explicit Author: Co-authored-by=True,
          --attribute-author -> Author=You(l2m), Committer=You, Trailer added
        - User commit (defaults): l2m_edits=False -> Author=You, Committer=You(l2m)
        - User commit with explicit no-committer: l2m_edits=False,
        ...
        prefix_commit_message = l2m_edits and (
        ...
        if l2m_edits and attribute_co_authored_by:
            model_name = "unknown-model"
            if coder and hasattr(coder, "main_model") and coder.main_model.name:
                model_name = coder.main_model.name
            commit_message_trailer = f"\n\nCo-authored-by: l2m ({model_name}) <l2m@l2m.chat>"

        # Determine if author/committer names should be modified
        # Author modification applies only to l2m edits.
        # It's used if effective_author is True AND
        # (co-authored-by is False OR author was explicitly set).
        use_attribute_author = (
            l2m_edits and effective_author and (not attribute_co_authored_by or author_explicit)
        )

        # Committer modification applies regardless of l2m_edits (based on tests).
        # It's used if effective_committer is True AND
        # (it's not an l2m edit with co-authored-by OR committer was explicitly set).
        use_attribute_committer = effective_committer and (
            not (l2m_edits and attribute_co_authored_by) or committer_explicit
        )
        ...
        if prefix_commit_message:
            commit_message = "l2m: " + commit_message
        ...
        committer_name = f"{original_user_name} (l2m)"
    ...
    def refresh_l2m_ignore(self):
        if not self.l2m_ignore_file:
            return

        current_time = time.time()
        if current_time - self.l2m_ignore_last_check < 1:
            return

        self.l2m_ignore_last_check = current_time

        if not self.l2m_ignore_file.is_file():
            return

        mtime = self.l2m_ignore_file.stat().st_mtime
        if mtime != self.l2m_ignore_ts:
            self.l2m_ignore_ts = mtime
            self.ignore_file_cache = {}
            lines = self.l2m_ignore_file.read_text().splitlines()
            self.l2m_ignore_spec = pathspec.PathSpec.from_lines(
    ...
    def ignored_file_raw(self, fname):
        ...
        if not self.l2m_ignore_file or not self.l2m_ignore_file.is_file():
            return False
        ...
        return self.l2m_ignore_spec.match_file(fname)
