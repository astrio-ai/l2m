...
from src.scrape import Scraper, has_playwright
...
class GUI:
    ...
    def show_edit_info(self, edit):
        ...
        if commit_hash:
            ...
            if commit_hash == self.coder.last_l2m_commit_hash:
                show_undo = True
    ...
    def initialize_state(self):
        ...
        self.state.init("last_l2m_commit_hash", self.coder.last_l2m_commit_hash)
    ...
    def process_chat(self):
        ...
        with self.messages:
            edit = dict(
                role="edit",
                fnames=self.coder.l2m_edited_files,
            )
            if self.state.last_l2m_commit_hash != self.coder.last_l2m_commit_hash:
                edit["commit_hash"] = self.coder.last_l2m_commit_hash
                edit["commit_message"] = self.coder.last_l2m_commit_message
                commits = f"{self.coder.last_l2m_commit_hash}~1"
                diff = self.coder.repo.diff_commits(
                    self.coder.pretty,
                    commits,
                    self.coder.last_l2m_commit_hash,
                )
                edit["diff"] = diff
                self.state.last_l2m_commit_hash = self.coder.last_l2m_commit_hash
    ...
    def do_undo(self, commit_hash):
        ...
        if (
            self.state.last_l2m_commit_hash != commit_hash
            or self.coder.last_l2m_commit_hash != commit_hash
        ):
