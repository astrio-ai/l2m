...
# L2M benchmark harness

L2M uses benchmarks to quantitatively measure how well it works
with various LLMs.
...
This
benchmark evaluates how effectively l2m and LLMs can translate a
...
and *format those code edits* so that l2m can save the
edits to the local source files.

See [this writeup for a longer discussion about the benchmark](https://l2m.chat/2024/12/21/polyglot.html).
...
There are 3 main tasks involved in benchmarking l2m:
...
```
# Clone the l2m repo
git clone https://github.com/L2M-AI/l2m.git

# Create the scratch dir to hold benchmarking results inside the main l2m dir:
cd l2m
mkdir tmp.benchmarks

# Clone the repo with the exercises
git clone https://github.com/L2M-AI/polyglot-benchmark tmp.benchmarks/polyglot-benchmark
...
# Inside the container, install l2m as a development build.
...
- `--model` is the name of the model, same as you would pass directly to `l2m`.
- `--edit-format` is the name of the edit format, same as you would pass directly to `l2m`. When working with an experimental LLM, I recommend starting with `whole`
...
- `--read-model-settings=<filename.yml>` specify model settings, see here: https://l2m.chat/docs/config/adv-model-settings.html#model-settings
...
[l2m leaderboard data files](https://github.com/L2M-AI/l2m/blob/main/l2m/website/_data/).
...
[l2m leaderboard data files](https://github.com/L2M-AI/l2m/blob/main/l2m/website/_data/).
- These scripts are not intended for use by typical l2m end users.
