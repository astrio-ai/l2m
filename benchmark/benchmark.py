...
def main(
    ...
    replay: str = typer.Option(
        None,
        "--replay",
        help="Replay previous .l2m.chat.history.md responses from previous benchmark run",
    ),
    ...
    no_l2m: bool = typer.Option(False, "--no-l2m", help="Do not run l2m"),
    ...
    read_model_settings: str = typer.Option(
        None, "--read-model-settings", help="Load l2m model settings from YAML file"
    ),
    ...
    if "L2M_DOCKER" not in os.environ:
    ...
    resource_metadata = importlib_resources.files("src.resources").joinpath("model-metadata.json")
...
def show_diffs(dirnames):
    ...
    for testcase in testcases:
        ...
        for outcome, dirname in zip(all_outcomes, dirnames):
            print(outcome, f"{dirname}/{testcase}/.l2m.chat.history.md")
...
def load_results(dirname, stats_languages=None):
    ...
    if stats_languages:
        ...
        glob_patterns = [f"{lang}/exercises/practice/*/.l2m.results.json" for lang in languages]
    else:
        glob_patterns = ["*/exercises/practice/*/.l2m.results.json"]
    ...
    for hsh in commit_hashes:
        ...
        try:
            version = subprocess.check_output(
                ["git", "show", f"{hsh}:l2m/__init__.py"], universal_newlines=True
            )
...
def get_replayed_content(replay_dname, test_dname):
    ...
    replay_fname = replay_dname / test_name / ".l2m.chat.history.md"
    dump(replay_fname)
...
def run_test(original_dname, testdir, *args, **kwargs):
    try:
        ...
        results_fname = testdir / ".l2m.results.json"
...
def run_test_real(
    ...
    history_fname = testdir / ".l2m.chat.history.md"
...
def run_unit_tests(original_dname, testdir, history_fname, test_files):
    ...
    TEST_COMMANDS = {
        ...
        ".go": ["go", "test", "./..."],
        ".js": ["/l2m/benchmark/npm-test.sh"],
        ".cpp": ["/l2m/benchmark/cpp-test.sh"],
        ".java": ["./gradlew", "test"],
    }
