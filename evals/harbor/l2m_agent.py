...
L2M is an installed agent that specializes in modernizing legacy codebases.
"""
...
class L2MAgent(BaseInstalledAgent):
    ...
    def create_run_agent_commands(self, instruction: str) -> list[ExecInput]:
        """
        Create the commands to run L2M in the container with iterative refinement.
        ...
        4. Iterative refinement loop: Run verifier tests and feed failures back to L2M
        """
        ...
        # This helps L2M understand what files exist before generating code
        pre_commands = []
        ...
        l2m_command = (
            f'l2m --message {escaped_enhanced} '
        ...
        # Iterative refinement loop: Run verifier tests and feed failures back to L2M
        ...
        # This ensures required output files are created even if L2M didn't generate them
        instruction_lower = instruction.lower()
        if 'test' in instruction_lower or 'sample' in instruction_lower or 'output' in instruction_lower or 'generate' in instruction_lower:
            # Create file generation script that runs after L2M
    ...
    def _create_file_generation_script(self, instruction_lower: str) -> str:
        ...
        This runs after L2M to ensure files are created even if L2M didn't generate them.
        """
        script_parts = [
            ...
            'fi',
            "",
            "# If files are still missing, ask L2M to generate them",
            'if [ ${#MISSING_REQUIRED_FILES[@]} -gt 0 ]; then',
            '    echo "=== Requesting L2M to generate missing files ==="',
            ...
            '    l2m --message "$MISSING_FILES_MSG" --map-tokens 2048 --yes-always 2>&1 || true',
            'fi',
            "",
    ...
    def _create_refinement_script(self, iteration: int, original_instruction: str) -> str:
        """
        Create a bash script that runs verifier tests and triggers L2M refinement if needed.
        
        The script:
        1. Finds and runs verifier test files (typically in /tests directory)
        2. Parses test results to identify failures
        3. If failures exist, runs L2M again with failure feedback
...
try:
    ...
    if "results" in data and "tests" in data["results"]:
        ...
        with open("/tmp/l2m_feedback_{iteration}.txt", "a") as feedback:
...
'''
        ...
        script_parts = [
            ...
            f'FEEDBACK_FILE="/tmp/l2m_feedback_{iteration}.txt"',
            "",
            ...
            "",
            "# Run L2M again with failure feedback",
            f'echo "=== Running L2M refinement (iteration {iteration + 1}) ==="',
            ...
            ")",
            "",
            "# Run L2M with enhanced feedback",
            f'l2m --message "$ENHANCED_FEEDBACK" --map-tokens 2048 --auto-test --yes-always',
            "",
