"""Evaluate L2M modernization results using CodeBLEU.

This script runs L2M to modernize COBOL files and then evaluates the generated
...
def run_l2m_modernization(
    ...
    """
    Run L2M modernization on a COBOL file.

    Args:
        cobol_file: Path to COBOL file to modernize.
        message: Custom message to send to L2M. If None, uses default modernization message.
    ...
    if message is None:
        ...
        # Include file path in message so L2M knows which file to convert
        try:
            cwd = Path.cwd()
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env=dict(os.environ, L2M_YES_ALWAYS="1"),  # Ensure yes-always is respected
        )
        success = result.returncode == 0
        output = result.stdout + result.stderr
        return success, output
    except subprocess.TimeoutExpired:
        return False, "L2M modernization timed out after 30 minutes"
    except FileNotFoundError:
        return False, "L2M command not found. Make sure 'l2m' is installed and in PATH."
    except Exception as e:
        return False, f"Error running L2M: {e}"


def evaluate_l2m_output(
    cobol_file: Path,
    ...
    """
    Run L2M modernization and evaluate with CodeBLEU.
    ...
    Args:
        ...
        run_modernization: Whether to run L2M modernization first. If False, assumes Python file already exists.
        l2m_message: Custom message for L2M. If None, uses default.
    ...
    # Step 1: Run L2M modernization (if requested)
    if run_modernization:
        print(f"Running L2M modernization on {cobol_file}...")
        success, output = run_l2m_modernization(cobol_file, message=l2m_message, output_dir=prediction_dir)
        if not success:
            return {
                "error": "L2M modernization failed",
                "l2m_output": output,
...
def main():
    """CLI entry point for L2M evaluation."""
    import argparse

    parser = argparse.ArgumentParser(
        description="Run L2M modernization and evaluate with CodeBLEU",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Modernize HELLO.cbl and compare with groundtruth
  python -m evals.codebleu.l2m_helpers data/hello/HELLO.cbl data/

  # Evaluate existing generated file (skip modernization)
  python -m evals.codebleu.l2m_helpers data/hello/HELLO.cbl data/ --no-modernize

  # Use custom output directory
  python -m evals.codebleu.l2m_helpers data/hello/HELLO.cbl data/ -o output/
        """,
    )
    ...
    parser.add_argument(
        "--no-modernize",
        action="store_true",
        help="Skip L2M modernization (assume Python file already exists)",
    )
    parser.add_argument(
        "--message",
        type=str,
    ...
    try:
        ...
        # Print results
        print("\n" + "=" * 60)
        print("L2M Modernization & CodeBLEU Evaluation Results")
        print("=" * 60)

        if "error" in result:
            print(f"Error: {result['error']}")
            if "l2m_output" in result:
                print(f"\nL2M output:\n{result['l2m_output']}")
            sys.exit(1)
