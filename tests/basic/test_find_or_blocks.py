...
class TestFindOrBlocks(unittest.TestCase):
    def test_process_markdown(self):
        ...
        # Skip test if fixture files are not available (they're gitignored due to size)
        import os
        if not os.path.exists(input_file) or not os.path.exists(expected_output_file):
            self.skipTest(
                "Large test fixtures not available. "
                "These files are gitignored due to size (4.9MB). "
                "To run this test, download the fixtures separately."
            )
